...
export default definePluginEntry({
  ...
  register(api) {
    api.registerTool({
      ...
      description:
        "Generate the latest plugin-usage snapshot from telemetry and serve the interactive network viewer on a localhost port. Use an SSH tunnel from your workstation to reach it.",
      parameters: Type.Object({
        windowDays: Type.Optional(
          Type.Number({ description: "Override the configured time window in days." })
        ),
        regenerateOnly: Type.Optional(
          Type.Boolean({ description: "Only refresh data.json, do not start the viewer server." })
        ),
      }),
      async execute(_id, params) {
        ...
        const sessionsDir =
          cfg.sessionsDir || path.join(os.homedir(), ".openclaw", "agents");
        ...
        let serve: Awaited<ReturnType<typeof ensureViewerServer>>;
        try {
