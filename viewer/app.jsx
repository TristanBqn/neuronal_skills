...
(function () {
    ...
    return (
      <div className="oc-tooltip" style={{ left: data.x + 14, top: data.y + 14 }}>
        ...
        <div className="oc-tt-meta">
          ...
          <span>·</span>
          <span>{p.usage > 0 ? `${pct}% active` : 'inactive'}</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <aside className="oc-side">
        ...
        <div className="oc-side-section is-flex">
          <div className="oc-side-label">Plugins by activity</div>
  ...
  function FocusCard({ plugin, links, onClose, onFocus }) {
    ...
    // also involved the other node. coTurns is l[3]; plugin.turns is this node's
    // distinct-turn count. (Ratios can overlap across neighbours since a turn may
    // involve 3+ tools, so they don't sum to 100%.)
    const connected = (links || [])
      ...
      .map((l) => {
        ...
        return {
          ...
          coTurns,
          ratio: plugin.turns ? coTurns / plugin.turns : 0,
        };
      })
      ...
      <div className="oc-focus">
        ...
        <div className="oc-focus-row">
          <div className="oc-focus-stat">
            <div className="oc-focus-stat-val">{Math.round(plugin.usage * 100)}%</div>
            <div className="oc-focus-stat-key">Singular use</div>
          </div>
          <div className="oc-focus-stat">
            <div className="oc-focus-stat-val">{plugin.files.length}</div>
            <div className="oc-focus-stat-key">Files</div>
          </div>
          <div className="oc-focus-stat">
            <div className="oc-focus-stat-val">{connected.filter((c) => c.coTurns > 0).length}</div>
            <div className="oc-focus-stat-key">Cross-talk</div>
          </div>
        </div>
  ...
  function App() {
    ...
    const onHover = useCallback((g, pt) => {
      setHovered(g);
      if (g && pt) setTooltip({ plugin: g, x: pt.x, y: pt.y });
    ...
    // Real event count from telemetry aggregation.
