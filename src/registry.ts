...
let cache: { at: number; data: PluginMeta[] } | null = null;
const TTL_MS = 60_000;

export async function loadRegistry(): Promise<PluginMeta[]> {
  ...
  const data: PluginMeta[] = normalize(raw);
  cache = { at: Date.now(), data };
  return data;
}

function normalize(raw: any): PluginMeta[] {
  if (!raw) return [];
  ...
  return list
    ...
    .filter((x): x is PluginMeta => x !== null);
}
...
function extractFiles(p: any): string[] {
  const candidates = [
    p.files,
    p.manifest?.files,
...
function extractTools(p: any): string[] {
  const candidates = [
    p.contracts?.tools,
...
export function buildToolToPluginIndex(registry: PluginMeta[]): Map<string, string> {
  const idx = new Map<string, string>();
  for (const p of registry) {
    for (const t of p.tools) {
      if (!idx.has(t)) idx.set(t, p.id);
    }
  }
  return idx;
}
