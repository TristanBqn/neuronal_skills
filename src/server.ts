...
export async function ensureViewerServer(opts: ServeOptions): Promise<ServeResult> {
  ...
  const shutdown = () => {
    if (idleTimer) clearTimeout(idleTimer);
    server.close();
    if (active && active.port === opts.port) active = null;
  };
  ...
  opts.logger?.info?.(
    `viewer server listening on http://${opts.host}:${opts.port}/ (idle shutdown ${opts.idleShutdownMs}ms)`
  );
...
function publicHost(bind: string): string {
  // Bind addresses like 0.0.0.0 / :: are not useful in a printed URL.
  if (bind === "0.0.0.0" || bind === "::" || bind === "::0") return "localhost";
  return bind;
}
